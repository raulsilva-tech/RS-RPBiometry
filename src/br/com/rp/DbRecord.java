...
public class DbRecord {
	 ...
	 * 
	 * @param szFileName
	 *            a file name with previous saved passport.
	 */
	public DbRecord(String szFileName) throws FileNotFoundException, NullPointerException, AppException {
		Load(szFileName);
	}
	 ...
	 *                szFileName parameter has null reference.
	 * @exception InvalidObjectException
	 *                the file has invalid structure.
	 * @exception FileNotFoundException
	 *                the file not found or access denied.
	 */
	private void Load(String szFileName) throws FileNotFoundException, NullPointerException, AppException {
		...
		try {
			...
			fs.read(Data);
			m_UserId = utf8Decoder.decode(ByteBuffer.wrap(Data)).toString();
	 ...
	 *                can not create file or can not write data into file.
	 */
	public boolean Save(String szFileName) throws NullPointerException, IllegalStateException, IOException {
		FileOutputStream fs = null;
		File f = null;
		boolean bRetcode = false;
		boolean bExist;

		if (m_Template == null || m_UserId == null)
		...
		try {
			...
			// Save user name
			ByteBuffer bBuffer = utf8Encoder.encode(CharBuffer.wrap(m_UserId.toCharArray()));
	...
	public int getIntUserId() {
		...
		if (m_UserId.isEmpty())
			...
			return Integer.parseInt(m_UserId);
	...
	static Vector<DbRecord> ReadRecords(String szDbDir) {
		...
		for (int iFiles = 0; iFiles < files.length; iFiles++) {
			try {
				...
				// The record has invalid data or access denied. Skip it and
				// continue processing.
			}
		}
